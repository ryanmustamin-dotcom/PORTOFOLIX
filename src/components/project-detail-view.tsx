...
export default function ProjectDetailView({ projectId, isModal = false }: ProjectDetailViewProps) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const commentsRef = collection(firestore, 'projects', projectId, 'comments');
  ...
  if (!project) return <div className="text-center py-20 font-subheadline">Karya tidak ditemukan.</div>;

  return (
    <div className={`container mx-auto ${isModal ? 'px-0' : 'py-10 px-4'}`}>
      <MessageDialog 
      ...
      <div className="grid grid-cols-12 gap-8">
        <div className={`col-span-12 ${isModal ? 'lg:col-span-12' : 'lg:col-span-9'}`}>
          <Card className="mb-8 border-none shadow-md bg-white">
            <CardContent className="p-6">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[10px] font-subheadline tracking-widest font-bold text-muted-foreground mb-6">
                ...
                {project.createdAt && (
                    <span className="hidden sm:inline bg-muted/30 px-3 py-1.5 rounded-full uppercase font-subheadline">• {project.createdAt.toDate().toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                )}
              </div>
            </CardContent>
          </Card>
